...
def save_data():
    ...
    with open(filename, mode="w", newline='') as file:
        ...
        writer.writerow(["Time", "U", "Y", "X1", "X2", "X3"])  # Escribir encabezado
...
def plot_data(filename):
    ...
    # Graficar X3
    plt.figure()
    plt.plot(time_data, x3_data, label='x3', color='blue')
    plt.title('Estado x3 en el tiempo')
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Valor de x3')
    plt.grid()
    plt.legend()
    plt.savefig(filename.replace('.csv', '_X3.png'))
    plt.close()
