...
%     end
    ...
    x_hat(:, k) = SS_disc.A * x_hat(:, k) + SS_disc.B * u(k-1);
    if u(k) > 4095
        u(k) = 4095;
    elseif u(k) < -4095
        u(k) = -4095;
    ...
    uu = u(k)*ones(1, numel(dt)); % ZOH de la U - DAC

    % Sistema en la vida real %
    [y, tsim, XssOut] = lsim(sys_ss, uu, dt, x(:, k)); % XssOut es la salida del Espacio de Estados. Cambiar
